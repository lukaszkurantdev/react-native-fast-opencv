...
jsi::Value Point2fVectorDelegate::get(jsi::Runtime& rt, const std::string& propName, const std::any& object) {
  switch(hashString(propName.c_str(), propName.size())) {
    case hashString("get", 3): {
      return jsi::Function::createFromHostFunction(
          ...
          [object](jsi::Runtime& runtime, const jsi::Value& thisVal, const jsi::Value* args, size_t count) -> jsi::Value {
              auto pointVectorPtr = std::any_cast<std::shared_ptr<std::vector<cv::Point2f>>>(object);
              auto point = pointVectorPtr->at(args[0].asNumber());
              return FOCV_JsiObject::wrap(runtime, "point2f", std::make_shared<decltype(point)>(point));
          }
      );
    }
      ...
      return jsi::Function::createFromHostFunction(
          ...
          [object](jsi::Runtime& runtime, const jsi::Value& thisVal, const jsi::Value* args, size_t count) -> jsi::Value {
              ...
              for (size_t i = 0; i < pointVectorPtr->size(); i++) {
                  ...
                  auto wrappedRect = FOCV_JsiObject::wrap(runtime, "point2f", std::make_shared<decltype(rect)>(rect));
    ...
    case hashString("push", 4): {
      return jsi::Function::createFromHostFunction(
          ...
          [object](jsi::Runtime& runtime, const jsi::Value& thisVal, const jsi::Value* args, size_t count) -> jsi::Value {
              auto pointVectorPtr = std::any_cast<std::shared_ptr<std::vector<cv::Point2f>>>(object);
              auto& object = *FOCV_JsiObject::get_from_wrap<cv::Point2f>(runtime, args[0]);
    ...
    case hashString("length", 6): {
      auto matPtr = std::any_cast<std::shared_ptr<std::vector<cv::Point2f>>>(object);
...
std::vector<jsi::PropNameID> Point2fVectorDelegate::getPropertyNames(jsi::Runtime& rt) {
...
size_t Point2fVectorDelegate::getExternalMemorySize(const std::any& object) {
  auto vectorPtr = std::any_cast<std::shared_ptr<std::vector<cv::Point2f>>>(&object);
  if (vectorPtr == nullptr || *vectorPtr == nullptr) {
    return 0;
  }
  return (*vectorPtr)->capacity() * sizeof(cv::Point2f);
}
