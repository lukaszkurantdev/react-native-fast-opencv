...
```js
OpenCV.absdiff(src1: Mat, src2: Mat, dst: Mat): void;
```
...
```js
OpenCV.add(src1: Mat, src2: Mat, dst: Mat): void;
OpenCV.add(src1: Mat, src2: Mat, dst: Mat, mask: Mat): void;
OpenCV.add(src1: Mat,
  src2: Mat,
  dst: Mat,
  mask: Mat,
...
```js
OpenCV.bitwise_and(src1: Mat, src2: Mat | Scalar, dst: Mat, mask?: Mat)
OpenCV.bitwise_and(src1: Scalar, src2: Mat, dst: Mat, mask?: Mat): void;
```
...
```js
OpenCV.bitwise_not(src: Mat, dst: Mat, mask?: Mat): void;
```
...
```js
OpenCV.bitwise_or(src1: Mat, src2: Mat | Scalar, dst: Mat, mask?: Mat): void;
OpenCV.bitwise_or(src1: Scalar, src2: Mat, dst: Mat, mask?: Mat): void;
```
...
```js
OpenCV.bitwise_xor(src1: Mat, src2: Mat | Scalar, dst: Mat, mask?: Mat)
OpenCV.bitwise_xor(src1: Scalar, src2: Mat, dst: Mat, mask?: Mat): void;
```
...
```js
OpenCV.borderInterpolate(p: number,
  len: number,
...
```js
OpenCV.calcCovarMatrix(samples: MatVector,
  nsamples: number,
  covar: Mat,
  mean: Mat,
  flags: CovarFlags,
...
```js
OpenCV.cartToPolar(x: Mat,
  y: Mat,
  magnitude: Mat,
  angle: Mat,
  angleInDegrees?: boolean
): void;
OpenCV.cartToPolar(x: MatVector,
  y: MatVector,
  magnitude: MatVector,
  angle: MatVector,
  angleInDegrees?: boolean
...
```js
OpenCV.checkRange(a: MatVector | Mat,
  quiet: boolean,
  pos: Point,
  minVal: number,
  maxVal: number
...
```js
OpenCV.compare(src1: Mat,
  src2: Mat | Scalar,
  dst: Mat,
  cmpop: CmpTypes
): void;
OpenCV.compare(src1: Scalar,
  src2: Mat,
  dst: Mat,
  cmpop: CmpTypes
): void;
...
```js
OpenCV.completeSymm(m: MatVector | Mat, lowerToUpper: boolean): void;
```
...
```js
OpenCV.convertFp16(src: Mat, dst: Mat): void;
```
...
```js
OpenCV.convertScaleAbs(src: Mat, dst: Mat, alpha?: number): void;
OpenCV.convertScaleAbs(src: Mat,
  dst: Mat,
  alpha: number,
  beta?: number
): void;
...
```js
OpenCV.convertTo(src: Mat, dst: Mat, rtype: DataTypes, alpha?: number, beta?: number): void;
```
...
```js
OpenCV.copyTo(src: Mat, dst: Mat, mask: Mat): void;
```
...
```js
OpenCV.countNonZero(src: MatVector | Mat): { value: number };
```
...
```js
OpenCV.crop(src: Mat, dst: Mat, roi: Rect): void;
```
...
```js
OpenCV.dct(src: Mat, dst: Mat, flags: DftFlags): void;
```
...
```js
OpenCV.determinant(src: Mat): { value: number };
```
...
```js
OpenCV.eigen(src: Mat, eigenvalues: Mat, eigenvectors: Mat): void;
```
...
```js
OpenCV.eigenNonSymmetric(src: Mat,
  eigenvalues: Mat,
  eigenvectors: Mat
): void;
```
...
```js
OpenCV.exp(src: Mat, dst: Mat): void;
```
...
```js
OpenCV.extractChannel(src: Mat, dst: Mat, coi: number): void;
```
...
```js
OpenCV.findNonZero(src: Mat, idx: Mat | PointVector): void;
```
...
```js
OpenCV.flip(src: Mat, dst: Mat, flipCode: number): void;
```
...
```js
OpenCV.getOptimalDFTSize(vecsize: number): { value: number };
```
...
```js
OpenCV.getRotationMatrix2D(center: Point2f, angle: number, scale: number, dst: Mat): void;
```
...
```js
OpenCV.hconcat(srcs: MatVector, dst: Mat): void;
```
...
```js
OpenCV.insertChannel(src: Mat, dst: Mat, coi: number): void;
```
...
```js
OpenCV.invert(src: Mat, dst: Mat, flags: DecompTypes): void;
```
...
```js
OpenCV.log(src: Mat, dst: Mat): void;
```
...
```js
OpenCV.LUT(src: Mat, lut: Mat, dst: Mat): void;
```
...
```js
OpenCV.magnitude(x: Mat, y: Mat, magnitude: Mat): void;
OpenCV.magnitude(x: MatVector, y: MatVector, magnitude: Mat): void;
```
...
```js
OpenCV.Mahalanobis(v1: Mat, v2: Mat, icovar: Mat): void;
OpenCV.Mahalanobis(v1: MatVector, v2: MatVector, icovar: Mat): void;
```
...
```js
OpenCV.max(src1: Mat, src2: Mat, dst: Mat): void;
```
...
```js
OpenCV.mean(src: Mat, mask?: Mat): Scalar;
```
...
```js
OpenCV.merge(mv: MatVector, dst: Mat): void;
```
...
```js
OpenCV.min(src1: Mat, src2: Mat, dst: Mat): void;
```
...
```js
OpenCV.merge(channels: MatVector, dst: Mat): void;
```
...
```js
OpenCV.patchNaNs(a: Mat | MatVector, val: number): void;
```
...
```js
OpenCV.perspectiveTransform(src: Mat, dst: Mat, m: Mat): void;
```
...
```js
OpenCV.pow(src: Mat, power: number, dst: Mat): void;
```
...
```js
OpenCV.PSNR(src1: Mat, src2: Mat, R: number): { psnr: number };
```
...
```js
OpenCV.repeat(src: Mat, ny: number, nx: number, dst: Mat): void;
```
...
```js
OpenCV.rotate(src: Mat, dst: Mat, code: RotateFlags): void;
```
...
```js
OpenCV.scaleAdd(src1: Mat, alpha: number, src2: Mat, dst: Mat): void;
```
...
```js
OpenCV.solveCubic(coeffs: Mat, roots: Mat): { value: boolean };
```
...
```js
OpenCV.solvePoly(src: Mat, dst: Mat, maxIters: number): { value: number };
```
...
```js
OpenCV.sort(src: Mat, dst: Mat, flags: SortFlags): void;
```
...
```js
OpenCV.sortIdx(src: Mat, dst: Mat, flags: SortFlags): void;
```
...
```js
OpenCV.split(src: Mat, dst: MatVector): void;
```
...
```js
OpenCV.sqrt(src: Mat, dst: Mat): void;
```
...
```js
OpenCV.subtract(src1: Mat,
  src2: Mat,
  dst: Mat,
  mask: Mat,
  dtype: DataTypes
): void;
OpenCV.subtract(src1: Mat, src2: Mat, dst: Mat, mask: Mat): void;
OpenCV.subtract(src1: Mat, src2: Mat, dst: Mat): void;
```
...
```js
OpenCV.sum(src: MatVector | Mat): Scalar;
```
...
```js
OpenCV.transform(src: Mat, dst: Mat, m: Mat): void;
```
...
```js
OpenCV.transpose(src: Mat, dst: Mat): void;
```
...
```js
OpenCV.vconcat(src: MatVector, dst: Mat): void;
```
...
```js
OpenCV.cvtColorTwoPlane(src1: Mat,
  src2: Mat,
  dst: Mat,
  code:
...
```js
OpenCV.cornerMinEigenVal(src: Mat,
  dst: Mat,
  blockSize: number
): void;
```
...
Feature detection and descriptor matching (ORB + Brute-Force matcher), plus a homography helper. See the [Feature matching & image registration](./examples/featurematching.md) example for a complete workflow.

### ORB_create

Creates an ORB (Oriented FAST and Rotated BRIEF) feature detector. All parameters are optional and default to OpenCV's defaults.

- nfeatures The maximum number of features to retain (default 500).
- scaleFactor Pyramid decimation ratio, greater than 1 (default 1.2).
- nlevels The number of pyramid levels (default 8).
- edgeThreshold Size of the border where features are not detected (default 31).
- firstLevel The level of pyramid to put the source image to (default 0).
- WTA_K The number of points that produce each element of the oriented BRIEF descriptor (default 2).
- scoreType `ORBScoreType.HARRIS_SCORE` (default) or `ORBScoreType.FAST_SCORE`.
- patchSize Size of the patch used by the oriented BRIEF descriptor (default 31).
- fastThreshold The FAST threshold (default 20).
...
- detector The feature detector (ORB).
- image Input image.
- mask Optional mask specifying where to look for keypoints.

Returns `{ keypoints: KeyPointVector, descriptors: Mat }`.
...
- normType Norm used for the distance between descriptors. Use `NormTypes.NORM_HAMMING` for ORB (binary) descriptors (default `NORM_HAMMING`).
- crossCheck If `true`, returns only mutually-best pairs (default `false`).

```js
OpenCV.BFMatcher_create(
  normType?: NormTypes,
  crossCheck?: boolean
): BFMatcher;
```
...
Finds the single best match for each query descriptor.

- matcher The descriptor matcher (BFMatcher).
- queryDescriptors Query descriptors.
- trainDescriptors Train descriptors.

Returns a `DMatchVector`.
...
Finds the k best matches for each query descriptor (useful for Lowe's ratio test).

- matcher The descriptor matcher (BFMatcher).
- queryDescriptors Query descriptors.
- trainDescriptors Train descriptors.
- k Count of best matches per query descriptor.

Returns a `DMatchVectorVector`.
...
Computes a perspective transform from exactly 4 matched point pairs, using `getPerspectiveTransform` internally.

> The `calib3d` module (and therefore `cv::findHomography` with RANSAC) is not available in the iOS OpenCV pod. This helper takes only the **first 4 points** of each vector, so the caller should pre-filter the matches down to the 4 best point pairs.

- srcPoints Source points (at least 4).
- dstPoints Destination points (at least 4).

Returns the 3x3 perspective transform as a `Mat`.
...
```js
OpenCV.bilateralFilter(src: Mat,
  dst: Mat,
  d: number,
  sigmaColor: number,
  sigmaSpace: number,
...
```js
OpenCV.getGaborKernel(ksize: Size,
  sigma: number,
  theta: number,
  lambd: number,
  gamma: number,
...
```js
OpenCV.getGaussianKernel(ksize: number,
  sigma: number,
  ktype: DataTypes.CV_32F | DataTypes.CV_64F
): Mat;
```
...
```js
OpenCV.getPerspectiveTransform(src: PointVector,
  dst: PointVector,
...
```js
OpenCV.getStructuringElement(shape: MorphShapes, ksize: Size): Mat;
OpenCV.getStructuringElement(shape: MorphShapes,
  ksize: Size,
  anchor: Point
): Mat;
```
...
```js
OpenCV.medianBlur(src: Mat, dst: Mat, ksize: number): void;
```
...
```js
OpenCV.morphologyDefaultBorderValue(): Scalar;
```
...
```js
OpenCV.morphologyEx(src: Mat, dst: Mat, op: MorphTypes, kernel: Mat): void;
OpenCV.morphologyEx(src: Mat, dst: Mat, op: MorphTypes, kernel: Mat, anchor: Point): void;
OpenCV.morphologyEx(src: Mat,
  dst: Mat,
  op: MorphTypes,
  kernel: Mat,
  anchor: Point,
...
```js
OpenCV.adaptiveThreshold(src: Mat,
  dst: Mat,
  maxValue: number,
...
```js
OpenCV.distanceTransform(src: Mat,
  dst: Mat,
...
```js
OpenCV.integral(src: Mat, sum: Mat): void;
```
...
```js
OpenCV.matchTemplate(image: Mat,
  templ: Mat,
  result: Mat,
...
```js
OpenCV.approxPolyDP(curve: Mat | MatVector | PointVector,
...
```js
OpenCV.arcLength(curve: Mat | MatVector | PointVector,
  closed: boolean
): { value: number };
```
...
```js
OpenCV.boundingRect(array: Mat | MatVector | PointVector): Rect;
```
...
```js
OpenCV.connectedComponents(image: Mat,
  labels: Mat
): { value: number };
```
...
```js
OpenCV.connectedComponentsWithStats(image: Mat,
  labels: Mat,
  stats: Mat,
  centroids: Mat
): { value: number };
...
```js
OpenCV.contourArea(contour: Mat | MatVector | PointVector,
  oriented?: boolean
): { value: number };
```
...
```js
OpenCV.convexHull(points: Mat, hull: Mat): void;
```
...
```js
OpenCV.convexityDefects(contour: Mat,
  convexHull: Mat,
...
```js
OpenCV.findContours(image: Mat | MatVector,
...
```js
OpenCV.findContoursWithHierarchy(image: Mat,
  contours: MatVector | PointVectorOfVectors,
  hierarchy: Mat,
  mode: RetrievalModes,
  method: ContourApproximationModes
...
```js
OpenCV.isContourConvex(contour: Mat): { value: boolean };
```
...
```js
OpenCV.matchShapes(contour1: Mat,
  contour2: Mat,
...
```js
OpenCV.minAreaRect(points: Mat): RotatedRect;
```
...
```js
OpenCV.warpPerspective(src: Mat,
  dst: Mat,
  M: Mat,
  size: Size,
