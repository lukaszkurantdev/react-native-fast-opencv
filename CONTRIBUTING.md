...
- When you make changes to `src/NativeFastOpencv.ts`.
...
To edit the Java or Kotlin files, open `example/android` in Android studio and find the source files at `react-native-fast-opencv` under `Android`.
...
```sh
Running "FastOpencvExample" with {"fabric":true,"initialProps":{"concurrentRoot":true},"rootTag":1}
```
